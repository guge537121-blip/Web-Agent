...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  function findDeepSeekSendButton(elements: Array<Record<string, unknown>>): Record<string, unknown> | undefined {
    ...
    for (let i = 0; i < elements.length; i++) {
      if (elements[i].kind === 'textarea') lastTextareaIdx = i
    }
  ...
  function findUserMessageInChat(elements: Array<Record<string, unknown>>, message: string): boolean {
  ...
  function isGeneratingResponse(elements: Array<Record<string, unknown>>): boolean {
    for (const el of elements) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const textarea = snap1.elements.find((el: Record<string, unknown>) => el.kind === 'textarea')
      if (!textarea) {
      ...
      const textareaAfter = snap3.elements.find((el: Record<string, unknown>) => el.kind === 'textarea')
