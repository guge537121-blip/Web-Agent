...
class RpcClient {
  ...
  async start(){const server=createServer();await new Promise((resolve,reject)=>{server.once('error',reject);server.listen(0,'127.0.0.1',resolve)});const address=server.address(),port=typeof address==='object'&&address?address.port:0,token=randomBytes(24).toString('hex');const child=spawn(this.executable,[this.hostMain,'--rpc-port',String(port),'--rpc-token',token],{stdio:['ignore','ignore','pipe'],windowsHide:false,env:{...process.env,DSH_WEB_AGENT_RPC_TOKEN:token}});child.stderr.setEncoding('utf8');child.stderr.on('data',d=>process.stderr.write(`[web-agent browser] ${String(d)}`));child.on('exit',(c,s)=>this.fail(new Error(`browser host exited (${String(c)}, ${String(s)})`)));this.socket=await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error('browser host connection timeout')),20000);server.once('connection',s=>{clearTimeout(timer);resolve(s)});child.once('error',reject)});server.close();this.socket.setEncoding('utf8');this.socket.on('data',d=>this.onData(String(d)));this.socket.on('error',e=>this.fail(e));this.socket.on('close',()=>this.fail(new Error('browser host connection closed')));const hello=await this.waitHello();if(hello.token!==token)throw new Error('browser host authentication failed');await this.call('ping');}
  waitHello(){return new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error('browser host hello timeout')),10000);const handler=data=>{for(const line of data.split('\n')){if(!line.trim())continue;try{const msg=JSON.parse(line);if(msg.op==='hello'){clearTimeout(timer);this.socket?.off('data',handler);resolve(msg);return}}catch{}}};this.socket?.on('data',handler)})}
...
class SharedElectronProvider {
  ...
  resolveElectron(){const explicit=process.env.ELECTRON_PATH;if(explicit&&existsSync(explicit))return explicit;const base=dirname(fileURLToPath(import.meta.url));const candidates=[join(base,'..','node_modules','electron','dist',process.platform==='win32'?'electron.exe':'electron'),join(base,'node_modules','electron','dist',process.platform==='win32'?'electron.exe':'electron'),join(process.cwd(),'node_modules','electron','dist',process.platform==='win32'?'electron.exe':'electron')];return candidates.find(existsSync)??candidates[0]}
